...
export default function EvalClient({ childId, moduleKey, childName }: EvalClientProps) {
  ...
  const loadNextQuestion = async (sessId: number) => {
    setStep('loading');
    try {
      const res = await getNextQuestionAction(sessId);

      if (res.ok) {
        if (res.done) {
          // Finalize session and get report
          const finalRes = await finaliseSessionAction(sessId);
          if (finalRes.ok) {
  ...
  const submitAnswer = async (e?: React.FormEvent) => {
    ...
    try {
      const res = await submitAnswerAction(sessionId, payload);
      if (res.ok) {
    ...
    <div className="max-w-2xl mx-auto py-4 px-4 space-y-6">
        ...
        <div className="card-premium bg-white dark:bg-slate-900 border border-slate-100 dark:border-slate-800 p-6 sm:p-8 space-y-6 animate-fade-in">
          <div className="text-center space-y-2 flex flex-col items-center">
            ...
            <p className="text-sm text-slate-500 max-w-sm mx-auto">
              You are about to start the <strong className="text-slate-700 dark:text-slate-350">{moduleKey}</strong> evaluation for <strong className="text-slate-700 dark:text-slate-350">{childName}</strong>.
            </p>
          </div>
          ...
          <div className="bg-indigo-50 dark:bg-slate-800/50 border border-indigo-100 dark:border-slate-850 rounded-2xl p-4 text-xs sm:text-sm text-indigo-900 dark:text-slate-300 space-y-2">
            ...
            <ul className="list-disc list-inside space-y-1 text-slate-500 dark:text-slate-400">
              <li>Ensure the room is quiet for voice prompts.</li>
              <li>Read or listen to the task together.</li>
              <li>Type or use the Microphone button to dictate answers.</li>
              <li>The evaluation will adapt based on correct responses.</li>
            </ul>
          </div>
      ...
      {step === 'loading' && (
        <div className="card-premium bg-white dark:bg-slate-900 border border-slate-100 dark:border-slate-800 p-12 text-center flex flex-col items-center justify-center space-y-4 min-h-[300px]">
          ...
          <p className="text-sm text-slate-500 animate-pulse">Loading next activity. Please wait...</p>
        </div>
      )}
      ...
      {step === 'memory_preview' && (
        <div className="card-premium bg-white dark:bg-slate-900 border border-slate-100 dark:border-slate-800 p-8 text-center flex flex-col items-center justify-center space-y-6 min-h-[350px] animate-fade-in">
          <span className="text-xs font-bold text-indigo-650 uppercase tracking-widest flex items-center gap-1">
            <Clock size={12} /> Memorise This Stimulus
          </span>
          ...
          <div className="space-y-1">
            <p className="text-sm text-slate-500 font-semibold">Hiding stimulus in:</p>
      ...
      {step === 'question' && currentQuestion && (
        <div className="space-y-4 animate-fade-in">
          ...
          <div className="bg-white dark:bg-slate-900 border border-slate-100 dark:border-slate-800 rounded-2xl p-4 flex justify-between items-center text-xs">
            <span className="font-bold text-slate-700 dark:text-slate-300">{childName}'s Evaluation</span>
            <span className="text-slate-400 font-semibold">
              Question {turnNo} of {totalTurns}
            </span>
          </div>
          ...
          <div className="card-premium bg-white dark:bg-slate-900 border border-indigo-100 dark:border-slate-800 p-6 sm:p-8 min-h-[350px] flex flex-col justify-between">
            <div className="space-y-6">
              ...
              {currentQuestion.stimulus && !currentQuestion.memory_mode && (
              ...
              <form onSubmit={submitAnswer} className="space-y-4">
                {currentQuestion.type === 'multiple_choice' ? (
                  <div className="grid grid-cols-1 gap-3">
                    {currentQuestion.choices?.map((ch: string) => {
                  ...
                  <div className="space-y-4">
                    <textarea
                      ...
                      placeholder="Type the answer here, or click the mic to speak..."
        ...
        <div className="space-y-6 animate-fade-in">
          ...
          {report.strengths && report.strengths.length > 0 && (
            <div className="card-premium bg-white dark:bg-slate-900 border border-slate-100 dark:border-slate-800 p-6 sm:p-8 shadow-sm">
              <h3 className="font-bold text-slate-800 dark:text-slate-100 text-lg mb-3">Key Strengths</h3>
      ...
      {step === 'error' && (
        <div className="card-premium bg-white dark:bg-slate-900 border border-slate-100 dark:border-slate-800 p-8 text-center space-y-4 animate-fade-in flex flex-col items-center">
          <AlertTriangle className="text-rose-600 mb-2" size={48} />
          <h2 className="heading-fun text-xl font-bold text-rose-800 dark:text-rose-400">
