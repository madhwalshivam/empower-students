...
function order_expert_report_via_wallet(int $parent_id, int $child_id): array {
    ...
    $existing = expert_report_status($parent_id, $child_id);
    if ($existing['id'] && in_array($existing['status'] ?? '', ['pending', 'delivered'], true)) {
        return [
...
/**
 * Get the latest delivered report for a child, if any.
