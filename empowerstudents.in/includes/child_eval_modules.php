...
LANGUAGE RULES (calibrated to child's mother tongue — see Child details below):
...
,
        ...
        // ─────────────────────────────────────────────────────────
        // EMOTIONS (child-facing version, NOT parent questionnaire)
        // The legacy /modules/emotions.php is a parent Likert form;
        // this engine version assesses the CHILD's emotional literacy.
        // ─────────────────────────────────────────────────────────
        'emotions' => [
            'turns' => 10,
            'axes' => [
            ...
            'system_prompt' => <<<'PROMPT'
You design ONE child-facing emotion question — child reads it on screen and answers.

Question types:

  "emotion_label"      — short scene description; child names the feeling ("happy", "frustrated", "lonely", etc.)
  "feeling_match"      — pick which face/emoji matches: 😊 😢 😠 😨 😳 🤔
...
- Age 4-7: use pictures of feelings (👶😊😢😠😨), simple sentences. Stimulus uses emoji.
...
,
            ...
            'scoring_system_prompt' => <<<'PROMPT'
Score the child's emotion answer:
...
,
            'report_system_prompt' => <<<'PROMPT'
Write a parent-facing report on the child's EMOTIONAL LITERACY.
...
,
        ...
        // ─────────────────────────────────────────────────────────
        // MATHS — adaptive level-based math screening
        // ─────────────────────────────────────────────────────────
        'maths' => [
            'turns' => 12,
            'axes' => [
            ...
            'system_prompt' => <<<'PROMPT'
You design ONE adaptive math question for the child. Screen-based, child types or picks an answer.
...
,
            'report_system_prompt' => <<<'PROMPT'
Parent-facing math report. Identify the level the child operates at (e.g. "operating at age-10 level", "needs strengthening on multiplication tables").
...
,
        ...
        // ─────────────────────────────────────────────────────────
        // LANGUAGE — vocabulary, reading, grammar, expression
        // ─────────────────────────────────────────────────────────
        'language' => [
            'turns' => 12,
            'axes' => [
            ...
            'system_prompt' => <<<'PROMPT'
You design ONE language question for the child. Screen-based.
...
CALIBRATION BY AGE:
- 4-5: pictures + sight words ("which one is a CAT? 🐱 🐶 🐰")
...
,
            'report_system_prompt' => <<<'PROMPT'
Report on language proficiency. State the level vs age expectation honestly.
...
,
        ...
        // ─────────────────────────────────────────────────────────
        // SPECIAL TALENT — discovery probe, NOT a skills test
        // Identifies interest areas + tiny demo of ability
        // ─────────────────────────────────────────────────────────
        'special_talent' => [
            'turns' => 10,
            'axes' => [
            ...
            'system_prompt' => <<<'PROMPT'
You design ONE talent-discovery question for the child. This is NOT pass/fail — you're discovering INTERESTS and SPARKS.
...
,
            ...
            'scoring_system_prompt' => <<<'PROMPT'
For talent discovery, scoring is NOT correctness — it's signal strength.
...
,
            'report_system_prompt' => <<<'PROMPT'
This is a TALENT DISCOVERY report. Tone is celebratory, not evaluative.
